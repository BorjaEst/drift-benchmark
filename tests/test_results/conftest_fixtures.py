...
def mock_failed_benchmark_result():
    ...
    # Mock configuration
    result.config = Mock()
    result.config.datasets = {"dataset1": Mock(path="data/dataset1.csv", drift_column="is_drift")}
    result.config.detectors = [
...
def sample_json_results_file(temp_workspace, mock_benchmark_result):
    ...
    json_content = {
        "config": {
            "datasets": {"dataset1": {"path": "data/dataset1.csv", "drift_column": "is_drift"}},
            "detectors": [{"method_id": "ks_test", "implementation_id": "scipy", "parameters": {"alpha": 0.05}}],
        },
        "detector_results": [
            {
                "method_id": "ks_test",
