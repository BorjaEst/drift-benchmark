...
def test_should_coordinate_data_loading_flow_when_benchmark_runs(mock_benchmark_config, mock_dataset_result):
    ...
    # Arrange
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert len(actual_calls) >= len(expected_calls), \
            "load_dataset should be called for each configured dataset"
...
def test_should_coordinate_detector_setup_flow_when_benchmark_runs(mock_benchmark_config, mock_detector):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert len(actual_detector_calls) >= len(expected_detector_calls), \
...
def test_should_coordinate_preprocessing_flow_when_benchmark_runs(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert hasattr(mock_detector_instance, 'preprocess'), \
            "detector must have preprocess method for data flow"
...
def test_should_coordinate_training_flow_when_benchmark_runs(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert mock_detector_instance._fitted, \
            "detector should be fitted during training phase"
        
        # Verify fit was called with reference data
        assert mock_detector_instance._fit_data is not None, \
            "detector fit should receive reference data"
...
def test_should_coordinate_detection_flow_when_benchmark_runs(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert mock_detector_instance._execution_count > 0, \
            "detector should perform detection during detection phase"
        
        # Verify detection was called after fitting
        assert mock_detector_instance._fitted, \
            "detection should only occur after training (fitting)"
...
def test_should_coordinate_scoring_flow_when_benchmark_runs(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert hasattr(result, 'detector_results'), \
            "benchmark result must include detector results for scoring"
        
        # Detector results should contain predictions and metadata for scoring
        if hasattr(mock_detector_instance, '_predictions'):
            assert mock_detector_instance._predictions is not None, \
                "detector should generate predictions for scoring"
...
def test_should_coordinate_result_storage_flow_when_benchmark_runs(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert hasattr(result, 'detector_results'), \
            "benchmark result must have structured detector_results"
        
        assert len(result.detector_results) > 0, \
            "detector_results should contain results for storage"
        ...
        for field in expected_fields:
            assert hasattr(detector_result, field), \
                f"DetectorResult must have {field} field for proper storage"
...
def test_should_coordinate_error_handling_flow_when_benchmark_runs(mock_benchmark_config, mock_failing_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        mock_get_detector.side_effect = lambda method_id, impl_id: \
            lambda *args, **kwargs: detector_factory(method_id, impl_id, **kwargs)
        ...
        assert result is not None, \
            "benchmark should complete despite individual detector failures"
        
        # Verify error statistics in result
        assert hasattr(result.summary, 'failed_runs'), \
            "result summary should track failed runs for error handling"
        
        assert hasattr(result.summary, 'successful_runs'), \
            "result summary should track successful runs for error handling"
...
def test_should_maintain_data_flow_isolation_when_benchmark_runs(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    dataset_configs = {
        'dataset1': Mock(path='data1.csv', drift_column='drift'),
        'dataset2': Mock(path='data2.csv', drift_column='drift')
    }
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert result is not None, \
            "data flow isolation should maintain benchmark completion"
