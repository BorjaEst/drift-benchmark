    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        def detector_factory(method_id, implementation_id, **kwargs):
            if method_id == "ks_test":
                return mock_detector(method_id, implementation_id, **kwargs)
            else:
...
def test_should_aggregate_results_when_run(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert len(result.detector_results) == expected_result_count, \
            f"should have {expected_result_count} detector results"
...
def test_should_track_execution_time_when_run(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        assert isinstance(result.summary.avg_execution_time, float), \
            "summary must include average execution time as float"
...
def test_should_maintain_detector_isolation_when_run(mock_benchmark_config, mock_detector, mock_dataset_result):
    ...
    with patch('drift_benchmark.data.load_dataset') as mock_load_dataset, \
        ...
        def detector_factory(method_id, implementation_id):
            if method_id == "ks_test":
        ...
        assert (result.summary.successful_runs + result.summary.failed_runs) == result.summary.total_detectors, \
            "all detector runs should be accounted for"
