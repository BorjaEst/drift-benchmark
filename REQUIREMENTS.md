...
| **REQ-DAT-008** | **Scenario Source Types**      | Data loading must support source_type="sklearn" for sklearn dataset generation, source_type="file" for CSV files in datasets_dir, and source_type="uci" for UCI ML Repository integration via ucimlrepo. For sklearn sources, source_name specifies the dataset function name. For file sources, source_name specifies the CSV filename. For UCI sources, source_name specifies the UCI dataset identifier |

### 📁 Enhanced Filtering System

| ID              | Requirement                     | Description                                                                                                                                                                                                                                             |
| --------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **REQ-DAT-009** | **Dataset Categorization**     | System must categorize sklearn datasets as SYNTHETIC_DATASETS = {"make_classification", "make_regression", "make_blobs"} and REAL_DATASETS = {"load_breast_cancer", "load_diabetes", "load_iris", "load_wine"} plus UCI_DATASETS for ucimlrepo integration                                      |
...
| **REQ-DAT-011** | **Real Dataset Preservation**  | Real datasets (load_* and UCI datasets) must never be modified. Drift is introduced only through intelligent sampling using feature-based filtering to leverage natural variation already present in the data                                                            |
...
| **REQ-DAT-016** | **Validation of Modifications** | System must validate that modification parameters (noise_factor, feature_scaling, etc.) are only applied to synthetic datasets, raising DataValidationError if used with real or UCI datasets                                                                 |
...
| **REQ-DAT-018** | **UCI Repository Integration** | System must support UCI ML Repository integration via ucimlrepo package, providing access to real-world datasets with comprehensive metadata including: total_samples, feature_types, missing_value_indicators, data_quality_score, and domain_information |

### 📁 Value Discovery Utilities

| ID              | Requirement                      | Description                                                                                                                                                                                                                                             |
| --------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **REQ-DAT-019** | **Threshold Discovery Interface** | Data module must provide `discover_feature_thresholds(dataset_name: str, feature_name: str) -> Dict[str, float]` returning statistical thresholds (min, max, median, q25, q75) for feature-based filtering                                          |
| **REQ-DAT-020** | **Dataset Feature Analysis**     | System must provide utilities to analyze feature distributions in real datasets to suggest meaningful filtering thresholds                                                                                                                              |
| **REQ-DAT-021** | **Baseline Scenario Validation** | Every drift scenario must have a corresponding no-drift baseline scenario for statistical validation. System must validate baseline exists for each drift scenario during configuration loading |
| **REQ-DAT-022** | **Quantitative Drift Measurement** | System must replace qualitative drift_intensity ("moderate", "severe") with quantitative metrics (kl_divergence, effect_size) in ground_truth sections of scenario files |
| **REQ-DAT-023** | **Feature Documentation**        | Value discovery utilities must provide descriptive information about features in real datasets to help users understand filtering implications                                                                                                           |
| **REQ-DAT-024** | **UCI Metadata Integration**     | System must extract and preserve UCI dataset metadata including: feature_names, feature_types (categorical/numerical), missing_value_count, data_quality_indicators, domain_context, acquisition_date, last_updated, and original_source_reference for scientific traceability |
| **REQ-DAT-025** | **Comprehensive Dataset Profiles** | All real-world datasets must include comprehensive profiles with: total_instances, feature_count, categorical_feature_count, numerical_feature_count, missing_data_percentage, class_distribution (if applicable), and data_collection_context |
